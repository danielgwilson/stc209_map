...
def save_animation2():
    ...
    for n in range(0, len(tstamps)):
        # bounds
        num_buildings = 92 # determined by maxi

        # init empty pixel array
        pixels = []
        for i in range(num_buildings):
            pixels.append([0,0,0,0,0])
        ...
        for index, row in dfms[tstamps[n]].iterrows():
            ...
            if k != 0:
                ...
                if ratio < midpoint:
                    ...
                    g = 0.1 * ratio * booster
                    r = ratio - midpoint * booster
                pixels[i] = [x, y, recast(r), recast(g), recast(b)]
                i = i + 1
        anim_txt.write(("%s,\n" % pixels).replace('[', '{').replace(']','}'))
...
# recast value for use in 0-255 array
def recast(v):
